...
| Rails | SmartCrab | 説明 |
|-------|-----------|------|
| `rails new` | `smartcrab new` | プロジェクト生成 |
| `rails generate model` | `smartcrab generate dto` | データ構造の生成 |
| `rails generate controller` | `smartcrab generate layer` | 処理単位の生成 |
| N/A | `smartcrab generate dag` | DAG 定義の生成 |
| `rails server` | `smartcrab run` | アプリケーション実行 |
...
`smartcrab new <project-name>` で生成されるプロジェクト構造:
...
`smartcrab generate layer <name>` で Layer のボイラープレートを生成する。
...
```bash
smartcrab generate layer webhook_receiver --type input --input-type http
smartcrab generate layer daily_check --type input --input-type cron
smartcrab generate layer discord_listener --type input --input-type chat
```
...
`smartcrab generate dto <name>` で DTO 構造体を生成する。
...
`smartcrab generate dag <name>` で DAG 定義関数のボイラープレートを生成する。
